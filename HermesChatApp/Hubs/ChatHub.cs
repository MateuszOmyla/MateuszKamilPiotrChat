...
public class ChatHub : Hub
{
    ...
    public async Task SendPrivateMessage(string toUser, string message)
    {
        ...
        User Sender = _chatRepository.GetUserByName(Context.User.Identity.Name);
        User Reciver = _chatRepository.GetUserByName(foundToUser.Name);
        Message messageToSaveInDatabase = new Message()
        {
            MessageFrom = Sender,
            MessageToUser = Reciver,
            MessageContent = message,
            MessageTime = DateTime.Now
        };

        _chatRepository.SaveMessage(messageToSaveInDatabase);   
    }
    ...
    public async Task SendMessageGroup(string toGroup, string message)
    {
        ...
        User Sender = _chatRepository.GetUserByName(Context.User.Identity.Name);
        Group GroupReciver = _chatRepository.GetGroupByName(toGroup);
        Message messageToSaveInDatabase = new Message()
        {
            MessageFrom = Sender,
            MessageToGroup = GroupReciver,
            MessageContent = message,
            MessageTime = DateTime.Now
        };
    ...
    public async Task CreateGroup(string groupName)
    {
        ...
        Group groupToSaveInDatabse = new Group()
        {
            GroupName = groupName
        };
        _chatRepository.AddGroup(groupToSaveInDatabse);
    }
